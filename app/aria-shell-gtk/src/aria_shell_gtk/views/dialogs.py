...
def show_control_tower_access_dialog(
    parent: Gtk.Widget,
    state: ClientState,
    service: ClientStore,
    on_success,
):
    _log_debug("show_control_tower_access_dialog start")
    ...
    _log_debug(f"show_control_tower_access_dialog root={type(root).__name__ if root is not None else 'None'}")
    win.set_modal(True)
    ...
    def close_window(*_args):
        _log_debug("show_control_tower_access_dialog close_window")
        win.destroy()
        return False
    ...
    def finish_auth(ok: bool, message: str) -> None:
        _log_debug(f"show_control_tower_access_dialog finish_auth ok={ok} message={message}")
    ...
    def google_signin(_btn=None):
        ...
        if not base_url:
            ...
            return
        _log_debug(f"show_control_tower_access_dialog google_signin base_url={base_url}")
    ...
    win.present()
    _log_debug("show_control_tower_access_dialog presented")
    return win


def show_openai_key_dialog(
    parent: Gtk.Widget,
    service: ClientStore,
    on_success,
):
