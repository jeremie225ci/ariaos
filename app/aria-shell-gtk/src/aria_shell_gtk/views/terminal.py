...
from ..services.bridge import SessionBridge
from ..services.history import HistoryService, SessionMessage, SessionSummary
from ..services.memo import compose_prompt as _build_session_prompt
from ..services.memo import memory_line as _session_memory_line
from ..services.store import ClientState, ClientStore
from .dialogs import show_control_tower_access_dialog, show_model_picker_dialog, show_openai_key_dialog
...
def _memory_line(role: str, raw_text: str) -> str:
    return _session_memory_line(role, raw_text, parse_assistant_payload=_parse_assistant_payload)


class TerminalView(Gtk.Box):
    def __init__(self, state: ClientState, service: ClientStore, bridge: SessionBridge, go_home):
    ...
    def _update_required_message(self) -> str:
        return (
            ...
            or "Update required. Download the new OVA to continue using AriaOS."
        )
    ...
    def _submit_prompt_payload(
        ...
        *,
        ...
        payload = self._compose_prompt(chosen_session_id, clean_prompt)
    ...
    def launch_remote_task(self, task: dict[str, object]) -> bool:
        ...
        elif self.state.update_required:
            self._sync_remote_task_blocked_async(task_id, session_id, self._update_required_message())
        return ok

    def refresh_state(self, state: ClientState) -> None:
    ...
    def _build_sidebar(self) -> None:
        ...
        brand = Gtk.Label(label="ARIA TERMINAL")
        ...
        self.plan_label = Gtk.Label(label="free")
        ...
        self.recommended_button = Gtk.Button(label="Recommended")
        ...
        self.account_button = Gtk.Button(label="Account")
        self.account_button.add_css_class("nav-button")
        self.account_button.connect("clicked", lambda _btn: self._open_account())
        sidebar.append(self.account_button)

        self.logout_button = Gtk.Button(label="Logout")
        self.logout_button.add_css_class("nav-button")
        self.logout_button.connect("clicked", lambda _btn: self._logout_account())
    ...
    def _build_workspace(self) -> None:
        ...
        self.strip_model_button.connect("clicked", lambda _btn: self._show_model_picker_dialog())
        ...
        self.overlay_kicker = Gtk.Label(label="CONTROL TOWER ACCESS")
        ...
        self.overlay_primary = Gtk.Button(label="Open Billing")
        self.overlay_primary.add_css_class("dock-send")
        self.overlay_secondary = Gtk.Button(label="Control Tower")
    ...
    def _runtime_ready(self) -> bool:
        if self._uses_remote_gateway():
            return (not self.state.update_required) and self.backend_connected
        return (not self.state.update_required) and self.state.api_key_ready and self.backend_connected
    ...
    def _set_connection_state(self) -> None:
        connected = (
            self.state.plan_status in {"active", "trialing"}
            and self._runtime_ready()
        )
        account_email = self.state.account_email if self.state.account_email and self.state.account_email != "Not connected" else ""
        if connected:
            label = f"connected · {account_email}" if account_email else "connected"
        else:
            label = f"disconnected · {account_email}" if account_email else "disconnected"
    ...
    def _plan_label_text(self) -> str:
        plan_name = (self.state.plan_name or "").strip()
        if self.state.plan_key == "annual_pro":
            return "annual pro"
        if self.state.plan_key == "monthly_pro":
            return "monthly pro"
        if self.state.plan_key == "enterprise":
            return "enterprise"
    ...
    def _maybe_show_activation_notice(self, previous_state: ClientState | None, new_state: ClientState) -> None:
    ...
    def _render_empty_hint(self, messages: list[SessionMessage]) -> None:
        ...
        if self.state.update_required:
            self.empty_hint.set_label(self._update_required_message())
        elif not self._selected_summary():
            self.empty_hint.set_label("Create a session from the left rail.")
        elif not self.state.auth_ready:
            self.empty_hint.set_label("Connect your Control Tower account to unlock Terminal Aria.")
        elif self.state.plan_status not in {"active", "trialing"}:
            self.empty_hint.set_label("Your plan is inactive. Open billing to unlock Terminal Aria.")
    ...
    def _render_overlay(self) -> None:
        for button, callback in (
            (self.overlay_primary, self._open_download_once),
            (self.overlay_primary, self._open_billing_once),
            (self.overlay_primary, self._open_key_dialog_once),
            (self.overlay_primary, self._open_unlock_once),
            (self.overlay_secondary, self._open_tower_once),
            (self.overlay_secondary, self._open_legacy_once),
            (self.overlay_secondary, self._refresh_access_once),
        ):
            try:
        ...
        if self.state.update_required:
            self.empty_box.set_visible(False)
            self.overlay_kicker.set_label("UPDATE REQUIRED")
            self.overlay_body.set_label(self._update_required_message())
            if self.state.latest_version:
                self.overlay_status.set_label(
                    f"Installed build {self.state.app_build}. Required build {self.state.min_supported_build}. Latest version: {self.state.latest_version}."
                )
            else:
                self.overlay_status.set_label(
                    f"Installed build {self.state.app_build}. Required build {self.state.min_supported_build}."
                )
            self.overlay_primary.set_label("Download latest OVA")
            self.overlay_secondary.set_label("Refresh Status")
            self.overlay_primary.connect("clicked", self._open_download_once)
            self.overlay_secondary.connect("clicked", self._refresh_access_once)
            self.state_overlay.set_visible(True)
            return
        if not self.state.auth_ready:
            self.empty_box.set_visible(False)
            self.overlay_kicker.set_label("CONTROL TOWER ACCESS")
            self.overlay_body.set_label(
                "Terminal Aria stays locked until this VM is linked to your Aria Control Tower account."
            )
            self.overlay_status.set_label("Sign in or create an account from here. Use Control Tower only for billing and account management.")
            self.overlay_primary.set_label("Connect Account")
            self.overlay_secondary.set_label("Open Control Tower")
            self.overlay_primary.connect("clicked", self._open_unlock_once)
            self.overlay_secondary.connect("clicked", self._open_tower_once)
            self.state_overlay.set_visible(True)
            return
        if self.state.plan_status not in {"active", "trialing"}:
            self.empty_box.set_visible(False)
            self.overlay_kicker.set_label("BILLING REQUIRED")
            if self.state.prompts_remaining == 0:
                self.overlay_body.set_label(
                    "Terminal Aria is locked because this account has used all 4 free prompts."
                )
                self.overlay_status.set_label("Subscribe in Control Tower to continue using Terminal Aria on this VM.")
            else:
                self.overlay_body.set_label(
                    "Terminal Aria is locked because this account does not have an active or trialing plan."
                )
                self.overlay_status.set_label("Open billing to subscribe or reactivate your plan, then return here.")
            self.overlay_primary.set_label("Open Billing")
            self.overlay_secondary.set_label("Refresh Access")
            self.overlay_primary.connect("clicked", self._open_billing_once)
            self.overlay_secondary.connect("clicked", self._refresh_access_once)
            self.state_overlay.set_visible(True)
            return
        if not self.state.api_key_ready and not self._uses_remote_gateway():
            self.empty_box.set_visible(False)
            self.overlay_kicker.set_label("LOCAL OPENAI KEY")
            self.overlay_body.set_label(
                "Connect your OpenAI key locally to unlock the GPT-5.4 or GPT-5.4 mini runtime from this terminal."
            )
            self.overlay_status.set_label("The key stays on this VM and the backend restarts locally.")
            self.overlay_primary.set_label("Connect Key")
            self.overlay_secondary.set_label("Open Billing")
            self.overlay_primary.connect("clicked", self._open_key_dialog_once)
            self.overlay_secondary.connect("clicked", self._open_billing_once)
            self.state_overlay.set_visible(True)
            return
        if not self.backend_connected:
            self.empty_box.set_visible(False)
            ...
            if self._uses_remote_gateway():
                ...
                self.overlay_body.set_label(
                    "Terminal Aria cannot reach the local backend. Start or reconnect the local OpenAI runtime first."
                )
    ...
    def _sync_controls(self) -> None:
        can_send = (
            not self.current_running
            and self.selected_session_id is not None
            and self.state.plan_status in {"active", "trialing"}
            and self._runtime_ready()
        )
        ...
        self.stop_button.set_sensitive(self.current_running)
        self.new_chat_button.set_sensitive(not self.current_running)
        self.recommended_button.set_sensitive(not self.current_running)
        self.account_button.set_sensitive(not self.current_running)
        self.logout_button.set_sensitive(not self.current_running)
        self.session_list.set_sensitive(not self.current_running)
        self.strip_access_button.set_sensitive(not self.current_running)
        self.strip_key_button.set_sensitive(not self.current_running)
        self.strip_model_button.set_sensitive(not self.current_running)
    ...
    def _compose_prompt(self, session_id: str, user_prompt: str) -> str:
        ...
        return _build_session_prompt(
            ...
            parse_assistant_payload=_parse_assistant_payload,
        )
    ...
    def _finalize_stream(
        ...
        if remote_task_id:
            self._sync_remote_task_async(
                ...
                latest_summary=final_text or response_raw or "Task finished.",
                latest_result=final_text if remote_status == "completed" else "",
                error=remote_error,
                spent_usd=float(usage.get("cost_usd") or 0.0) if isinstance(usage, dict) else None,
                usage=usage if isinstance(usage, dict) else None,
                event_text=final_text or response_raw or "Task finished.",
    ...
    def _show_model_picker_dialog(self) -> None:
        show_model_picker_dialog(self, self.service, lambda: self.refresh_state(self.service.read()))

    def _show_access_dialog(self) -> None:
        self._log_debug(
            f"_show_access_dialog entered access_window={'set' if self.access_window is not None else 'none'} "
            f"auth_ready={self.state.auth_ready} account_email={self.state.account_email}"
        )
        if self.access_window is not None:
            self._log_debug("_show_access_dialog present existing window")
            self.access_window.present()
            return

        def on_success():
            self._log_debug("_show_access_dialog on_success")
            self.access_window = None
            self.state = self.service.read()
            self.refresh_state(self.state)

        self.access_window = show_control_tower_access_dialog(self, self.state, self.service, on_success)
        self._log_debug(f"_show_access_dialog created window={self.access_window is not None}")
    ...
    def _open_account(self) -> None:
        self._log_debug(
            f"_open_account auth_ready={self.state.auth_ready} "
            f"account_email={self.state.account_email}"
        )
        if self.state.auth_ready:
            self.service.open_account()
            return
