...
from .state import ShellState, StateService
from .remote_runtime import RemoteComputerRuntime
from .runtime_contract import (
    MSG_FINAL_RESPONSE,
    MSG_COMPUTER_ACTION_REQUEST,
    MSG_COMPUTER_ACTION_RESULT,
    MSG_COMPUTER_SCREENSHOT_REQUEST,
    MSG_COMPUTER_SCREENSHOT_RESULT,
    MSG_START_TASK,
    MSG_STATUS_UPDATE,
    MSG_STOP_TASK,
    MSG_PAUSE_TASK,
...
class RemoteTransportBridge(threading.Thread):
    ...
    async def _sender(self, ws) -> None:
        while not self.shutdown_event.is_set():
            ...
            ok, prepared = await asyncio.to_thread(
                self.service.prepare_remote_task_start,
                task_id=task_id,
    ...
    async def _receiver(self, ws) -> None:
        while not self.shutdown_event.is_set():
            ...
            if msg_type == MSG_COMPUTER_ACTION_REQUEST:
                ...
                await self._send_json(
                    ws,
                    {
                        "type": MSG_COMPUTER_ACTION_RESULT,
            ...
            if msg_type == MSG_COMPUTER_SCREENSHOT_REQUEST:
                request_id = str(data.get("requestId") or "")
                ...
                await self._send_json(
                    ws,
                    {
                        "type": MSG_COMPUTER_SCREENSHOT_RESULT,
                        "taskId": str(data.get("taskId") or self._active_task_id or ""),
                        "requestId": request_id,
                        "ok": bool(ok),
            ...
            if msg_type == MSG_VM_TOOL_REQUEST:
                request_id = str(data.get("requestId") or "")
            ...
            if msg_type == MSG_FINAL_RESPONSE:
    ...
    async def _loop(self) -> None:
        gateway_url = self.state.remote_gateway_ws_url.strip()
        if websockets is None:
            self.emit({"type": "status", "text": "remote gateway unavailable: install websockets"})
            return
        while not self.shutdown_event.is_set():
            try:
                self.emit({"type": "status", "text": f"connecting {gateway_url}"})
                ...
                self.emit({"type": "status", "text": f"remote gateway reconnect: {exc}"})
