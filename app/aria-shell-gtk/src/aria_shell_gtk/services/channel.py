...
Q5 = "q"
S0 = "No active task."
S1 = "Pausing current task."
S2 = "Resuming current task."
S3 = "Request rejected."
S4 = "Request failed."
S5 = "Link unavailable."
S6 = "Connecting..."
...
class TaskChannel(threading.Thread):
    def __init__(self, state: RuntimeState):
        ...
        self._realtime_token: str = ""
        self._realtime_uid: str = ""
        self._realtime_device_id: str = ""
        self._realtime_machine_uid: str = ""
        self._realtime_device_name: str = ""
    ...
    def request_pause(self) -> bool:
        if not self._active_task_id or not self._active_session_id:
            self.emit({"type": "system", "message": S0})
            return False
        self.emit({"type": "system", "message": S1})
        self.task_queue.put({"type": Q3})
        return True
    ...
    def request_resume(self) -> bool:
        if not self._active_task_id or not self._active_session_id:
            self.emit({"type": "system", "message": S0})
            return False
        self.emit({"type": "system", "message": S2})
        self.task_queue.put({"type": Q4})
        return True

    def shutdown(self) -> None:
        self.shutdown_event.set()
        self.task_queue.put({"type": Q5})

    def _current_vm_status(self) -> str:
        if self._active_task_id:
            return "running"
        return "ready"

    def _ensure_realtime_token(self) -> bool:
        if self._realtime_token and self._realtime_uid and self._realtime_device_id:
            return True
        ok, payload = self.service.fetch_realtime_token(kind="device")
        if not ok or not isinstance(payload, dict):
            message = str((payload or {}).get("message") or S5)
            self.emit({"type": "status", "text": f"{S7}: {message}"})
            return False
        self._realtime_token = str(payload.get("token") or "").strip()
        self._realtime_uid = str(payload.get("uid") or "").strip()
        self._realtime_device_id = str(payload.get("device_id") or "").strip()
        self._realtime_machine_uid = str(payload.get("machine_uid") or "").strip()
        self._realtime_device_name = str(payload.get("device_name") or "Aria VM").strip() or "Aria VM"
        return bool(self._realtime_token and self._realtime_uid and self._realtime_device_id)

    async def _register_realtime(self, ws) -> None:
        if not self._ensure_realtime_token():
            return
        await self._send_json(
            ws,
            {
                "type": "register_rt",
                "realtimeToken": self._realtime_token,
                "currentTaskId": str(self._active_task_id or ""),
                "vmStatus": self._current_vm_status(),
            },
        )
        ...
        while not self.shutdown_event.is_set():
            ...
            if not ok:
                self.emit(
                    {
                        "type": "response",
                        "text": str(prepared.get("message") or S3),
                        "session_id": self._active_session_id,
                        "task_id": task_id,
                    }
                )
    ...
    async def _receiver(self, ws) -> None:
        while not self.shutdown_event.is_set():
            ...
            if msg_type == "remote_task_start":
                task = data.get("task") if isinstance(data.get("task"), dict) else None
                if task:
                    self.emit({"type": "remote_task_start", "task": task})
                continue

            if msg_type == "remote_task_stop":
                self.emit({"type": "remote_task_stop", "task_id": str(data.get("taskId") or "")})
                continue

            if msg_type == "remote_task_pause":
                self.emit({"type": "remote_task_pause", "task_id": str(data.get("taskId") or "")})
                continue

            if msg_type == "remote_task_resume":
                self.emit({"type": "remote_task_resume", "task_id": str(data.get("taskId") or "")})
                continue

            if msg_type == "remote_task_message":
                message = data.get("message") if isinstance(data.get("message"), dict) else None
                if message:
                    self.emit(
                        {
                            "type": "remote_task_messages",
            ...
            if msg_type == "remote_task_messages":
            ...
            if msg_type == "vm_link_connect":
                payload = data.get("payload") if isinstance(data.get("payload"), dict) else {}
                ok, message = self.service.apply_account_link_payload(payload)
                self.emit(
                    {
                        "type": "system",
                        "text": "vm_link_connected" if ok else f"vm_link_failed:{message}",
                    }
                )
                continue
            ...
            if msg_type == MF:
                ...
                payload = data.get("payload") if isinstance(data.get("payload"), dict) else {}
                ok = False
                detail = f"unsupported_op:{operation}"
            ...
            if msg_type == MD:
                self.emit(
                    {
                        "type": "response",
                        "text": str(data.get("reason") or S4),
                        "session_id": self._active_session_id,
                        "task_id": self._active_task_id,
                        "outcome": "failed",
                    }
    ...
    async def _loop(self) -> None:
        ...
        if websockets is None:
            self.emit({"type": "status", "text": S5})
            return
        while not self.shutdown_event.is_set():
            try:
                self.emit({"type": "status", "text": S6})
                async with websockets.connect(
                    ...
                    await self._register_realtime(ws)
                ...
                self.emit({"type": "status", "text": f"{S7}: {exc}"})
