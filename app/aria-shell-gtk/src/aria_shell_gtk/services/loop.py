...
from .codec import L0, L1, L2, L3, L4, L5, L6, L7, L8, L9
...
class VisionPowerManager:
    def __init__(self, state: RuntimeState, ui_queue: queue.Queue):
        self.state = state
...
class LocalLoop(threading.Thread):
    ...
    async def _wait_for(self, ws, request_id: str, expected_type: str, timeout: float = 8.0) -> dict[str, Any] | None:
        ...
        while time.time() < deadline:
            try:
                ...
                continue
            data = json.loads(raw)
            if data.get("requestId") == request_id and _is_local_type(data.get("type"), expected_type, {
                L1: "new_session_result",
                L3: "generate_title_result",
            }.get(expected_type, expected_type)):
                return data
            if _is_local_type(data.get("type"), L9, "proactive_action"):
    ...
    async def _reset_local_session(self, ws, session_id: str, timeout: float = 8.0) -> bool:
        ...
        reply = await self._wait_for(ws, request_id, L1, timeout=timeout)
        if not reply:
            self.emit({"type": "system", "message": "[session] reset timeout, continuing"})
            return False
        self.emit({"type": "system", "message": "[session] ready"})
        return True
    ...
    async def _request_session_title(self, ws, session_id: str, text: str, timeout: float = 8.0) -> str | None:
        ...
        reply = await self._wait_for(ws, request_id, L3, timeout=timeout)
        if not reply:
            return None
    ...
    async def _run_single_goal(self, ws, session_id: str, prompt: str, image: dict[str, Any] | None = None) -> None:
        ...
        try:
            while True:
                ...
                if _is_local_type(msg_type, L9, "proactive_action"):
                    self.emit({"type": "proactive", "data": data.get("data", {})})
                    continue

                if _is_local_type(msg_type, L5, "stream") and rid == request_id:
                    chunk = data.get("chunk", "")
                    if chunk:
                        self.emit({"type": "stream", "chunk": chunk, "session_id": session_id})
                    continue

                if _is_local_type(msg_type, L6, "response") and rid == request_id:
                    self.emit(
                        {
                            "type": "response",
                ...
                if _is_local_type(msg_type, L8, "stop_result"):
    ...
    async def _loop(self) -> None:
        while not self.shutdown_event.is_set():
            try:
                self.emit({"type": "status", "text": f"connecting {self.state.local_url}"})
                async with websockets.connect(
                    self.state.local_url,
                    ping_interval=20,
                    ping_timeout=30,
                    max_size=8_000_000,
                ) as ws:
