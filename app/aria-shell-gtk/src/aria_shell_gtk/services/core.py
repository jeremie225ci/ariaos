...
from .prompts import LONG_TERM_MEMORY_DECISION_PROMPT
...
def _ensure_memory_index_synced(path: Path, runtime: dict[str, str] | None) -> bool:
    # The human-readable memory file stays in Markdown on the desktop, but
    # semantic recall is driven by a local SQLite index of summary embeddings.
    # This sync step keeps the index aligned with the current summaries before
...
def _retrieve_semantic_memory_summaries(
    ...
    *,
    ...
    # Retrieval is hybrid on purpose: embeddings rank semantically related task
    # summaries, then keyword overlap and section bias help stable lines survive
    # even when two summaries are close in vector space.
...
class RuntimeStore:
    ...
    def apply_long_term_memory_action(action: dict[str, Any], *, runtime: dict[str, str] | None = None) -> dict[str, Any]:
        ...
        # The decision model only chooses the mutation. This method is the
        # single write path that applies the mutation to the Markdown memory
        # file, then re-compacts and re-indexes the local semantic store.
    ...
    def decide_long_term_memory_action(
        ...
        # Aria still records something useful without an API key: durable user
        # facts and completed-task summaries can be appended locally without
        # asking a second model to classify the outcome.
        ...
        try:
            # The memory decision prompt is separate from the execution brain:
            # it decides whether the task outcome should become a preference,
            # playbook, summary, update, deletion, or no-op.
            decision = _openai_chat_json(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": LONG_TERM_MEMORY_DECISION_PROMPT,
                    },
                    {
                        "role": "user",
                        "content": (
    ...
    def _spawn_local_service(
        *,
        ...
        # The public VM should expose one readable source tree, not a growing
        # set of generated bytecode mirrors beside it.
