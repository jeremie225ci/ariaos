"""
AriaOS v3 — Brain Configuration
================================
The reinforced system prompt + all configuration constants.
This is the BRAIN's DNA — it tells the AI what it IS and what it CAN do.
"""
...
try:
    from config import OPENAI_API_KEY as _LEGACY_OPENAI_API_KEY
except Exception:
    _LEGACY_OPENAI_API_KEY = ""
...
OPENAI_MODEL = (
    os.environ.get("ARIA_OPENAI_MODEL")
...
MAX_HISTORY_MESSAGES = 50       # Store last 50 exchanges on disk
CONTEXT_WINDOW_SIZE = 10        # Load last 10 exchanges as context per task

# ============================================================
# Agentic Loop Configuration
# ============================================================
MAX_AGENTIC_STEPS = 0           # 0 = no hard step limit
COMMAND_TIMEOUT = 45            # Timeout per command (seconds) — allows longer installs
STEP_DELAY = 0.3                # Delay between steps (seconds)

# ============================================================
# Perception Configuration (Routing Order)
# ============================================================
PERCEPTION_LAYERS = [
    "vision",       # Visual-only mode for the local-first public build
]
...
# ============================================================
# Sniper / legacy remote vision stack is disabled in the local-first public build.
# ============================================================
SNIPER_API_KEY = ""
SNIPER_BASE_URL = ""
SNIPER_MODEL = ""
...
# ============================================================
SLIDING_WINDOW_RAW_MESSAGES = 8    # Last N raw messages sent to GPT planner
...
# Input Configuration
# ============================================================
# Default to xdotool reliability in VM; set ARIA_PREFER_KERNEL_INPUT=true to force /dev/uinput.
PREFER_KERNEL_INPUT = os.environ.get("ARIA_PREFER_KERNEL_INPUT", "false").lower() in ("1", "true", "yes", "on")
...
# ============================================================
TRACE_ENABLED = os.environ.get("ARIA_TRACE_ENABLED", "true").lower() in ("1", "true", "yes", "on")
TRACE_FILE = os.environ.get("ARIA_TRACE_FILE", "/tmp/ariaos_agent_trace.jsonl")
...
CRITICAL: CDP (Chrome DevTools Protocol) is DISABLED. You are in visual-only mode. Do not attempt to use CDP or read the DOM. Use your eyes (vision / screen perception) to perceive the screen.
...
- PRIVILEGE RULE: This VM is expected to grant sudo inside the VM without storing an admin password in AriaOS files. Run the privileged command you need directly and only conclude sudo is unavailable after a real privileged command fails.
...
{"thought": "This is a future task, but the execution time is missing. I must ask for clarification in the user's language and not execute anything now.", "progress_log": "Need scheduling clarification", "action": "respond", "text": "À quelle heure exacte veux-tu que je le fasse ?"}
...
# ============================================================
# Navigator System Prompt (for app_navigator.py)
# ============================================================
