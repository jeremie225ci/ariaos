...
The project was initially created to satisfy my curiosity. I’ve always wondered what would happen if I put an AI agent in a VM, gave it full privileges—including sudo—and let it interact with the computer’s user interface (clicking, scrolling, etc.). Well, to carry out this project, the only feasible way was to place the agent in a virtual machine on a dedicated Linux system. So I went ahead and created a Debian fork dedicated to the agent, explicitly choosing GPT 5.4 because it’s relatively cost-effective and has the best computer usage scores in benchmark tests. The results are quite impressive and mind-blowing: the agent is capable of building entire complex apps in a matter of moments, can use my compromised apps, and operates on certain computers with highly sophisticated reasoning.
